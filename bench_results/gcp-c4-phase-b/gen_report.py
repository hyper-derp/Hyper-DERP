#!/usr/bin/env python3
"""Generate Phase B (kTLS) sweep report with plots."""
...
CONFIGS = ["16vcpu", "8vcpu", "4vcpu", "2vcpu"]
CONFIG_LABELS = {
  "16vcpu": "16 vCPU (8w)",
  "8vcpu": "8 vCPU (4w)",
  "4vcpu": "4 vCPU (2w)",
...
def plot_throughput_all():
  if not HAS_MPL:
    return
  configs = ["2vcpu", "4vcpu", "8vcpu", "16vcpu"]
  ...
  fig.suptitle(
    "Throughput with TLS — GCP c4-highcpu (n=25)",
...
def plot_loss_all():
  ...
  fig.suptitle(
    "Message Loss with TLS — GCP c4-highcpu (n=25)",
    fontsize=14, fontweight="bold")
  plt.tight_layout()
...
def generate_report():
  ...
  w("# Phase B Report — kTLS Full Sweep")
  w("")
  w("**Date**: 2026-03-15")
  ...
  w("**TS build**: v1.96.1 release, Go 1.26.1, TLS")
  w("")
  ...
  w("### 2. kTLS vs userspace TLS")
  w("")
  w("HD uses kernel TLS (kTLS) — AES-GCM runs in the kernel,")
  w("worker threads never touch crypto. TS uses Go's crypto/tls")
  w("in userspace — every goroutine does its own encryption.")
  w("")
  w("At 16 vCPU, both have enough CPU for crypto — ratio is 1.6x.")
  w("At 2 vCPU, TS goroutines compete for crypto CPU — ratio is")
  w("11.8x. kTLS is the architectural advantage that matters most")
  w("when resources are scarce.")
  w("")

  w("### 3. 2 vCPU: the headline number")
  w("")
  w("At 5 Gbps offered:")
  w("- HD delivers 2,977 Mbps (2.8% loss)")
  w("- TS delivers 252 Mbps (93% loss)")
  w("- **11.8x throughput ratio**")
  w("")
  w("TS on 2 vCPU with TLS is effectively unusable above 1.5 Gbps.")
  w("HD on 2 vCPU delivers nearly 3 Gbps — exceeding what TS")
  w("delivers on 8 vCPU (4 Gbps) with 75% less hardware.")
  w("")

  w("### 4. Latency at 2 vCPU is the strongest result")
  w("")
  w("At 150% of TS ceiling:")
  w("- HD p99: 954 us")
  w("- TS p99: 7,992 us")
  w("- **8.4x lower tail latency**")
  w("")
  w("HD is still relaying traffic at this load. TS has collapsed")
  w("(93% loss) — the low p50 for TS at high load is an artifact")
  w("of only successfully processing a trickle of packets.")
  w("")

  w("![Phase A vs B](plots/phase_a_vs_b.png)")
  w("")
