...
Ending up with a shard-per-core, share-nothing design. This is basically what Seastar (ScyllaDB's framework) does, if you go down the path of removing all possible context switches from the forwarding path this is where you end up.
...
{{< plot src="tunnel_scaling.png" alt="UDP throughput through WireGuard tunnel at increasing offered rates. Both relays track together, capped by WireGuard crypto at ~2 Gbps." >}}
...
**NIC TLS offload.** The bare metal profiling proved that kTLS consumes 25% of CPU cycles and costs 27-48% of throughput. ConnectX-5/6 with hardware kTLS offload would eliminate the crypto tax entirely, including the cache cliff and kTLS-driven variance. For the next hardware purchase: TLS offload capability over raw line rate.

**eBPF steering.** Currently, all connections land on the accept thread and get assigned to workers via FNV-1a hash. An eBPF XDP program could steer incoming connections directly to the correct worker's io_uring, eliminating the accept thread as a serialization point.
