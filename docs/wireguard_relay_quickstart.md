...
If you want to chase throughput on real silicon (10/25 GbE NICs, cloud) there's an XDP fast-path mode — see the bench reports in the [`HD-bench-data`](https://github.com/hyper-derp/HD-bench-data) repo (`wireguard_relay_bench.md`, `wireguard_relay_bench_25g.md`, `wireguard_relay_bench_gcp.md`). For homelab speeds the userspace path is fine.
